...
async function fetchFromOpenFoodFacts(upc: string): Promise<APIProductResponse | null> {
  try {
    const response = await fetch(`${OPEN_FOOD_FACTS_BASE}/product/${upc}.json`, {
      headers: {
...
async function fetchFromUPCItemDB(upc: string): Promise<APIProductResponse | null> {
  try {
...
export function generateMockProduct(upc: string): APIProductResponse {
  return {
    ...
    name: 'Demo Product',
    brand: 'Demo Brand',
    ingredients: 'Water, Enriched Wheat Flour (Wheat Flour, Niacin, Reduced Iron, Thiamine Mononitrate, Riboflavin, Folic Acid), Sugar, Natural Flavors, Salt, Yeast Extract, Citric Acid',
    imageUrl: undefined,
    source: 'demo',
  };
}