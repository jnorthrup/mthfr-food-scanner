...
import { ArrowLeft, History, Heart, Trash2, Filter, SortAsc, Search, Package } from 'lucide-react';
...
export function HistoryScreen() {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'safety': {
          ...
          return priority[getProductSafetyStatus(a)] - priority[getProductSafetyStatus(b)];
        }
        case 'recent':
        default:
          return new Date(b.lastScannedAt || b.createdAt).getTime() - 
  ...
  if (currentProduct) {
    ...
    return (
      <div data-design-id="product-detail" className="h-full flex flex-col bg-background">
        <div data-design-id="detail-header" className="p-4 flex items-center gap-3 border-b">
          <Button
          ...
          <h2 className="font-semibold flex-1 truncate">{currentProduct.name}</h2>
          <Button
        ...
        <ScrollArea className="flex-1">
          <div className="p-5 space-y-6 pb-24">
            ...
            <div data-design-id="product-meta" className="text-xs text-muted-foreground space-y-1">
              <p>UPC: {currentProduct.upc}</p>
              <p>Source: {currentProduct.sourceProvenance}</p>
              <p>Added: {new Date(currentProduct.createdAt).toLocaleDateString()}</p>
              {currentProduct.lastScannedAt && (
                <p>Last scanned: {new Date(currentProduct.lastScannedAt).toLocaleDateString()}</p>
              )}
            </div>
          </div>
        </ScrollArea>
  ...
  return (
    <div data-design-id="history-screen" className="h-full flex flex-col bg-background">
      <div data-design-id="history-header" className="p-4 border-b space-y-4">
        <div className="flex items-center justify-between">
          <h1 data-design-id="history-title" className="text-xl font-bold">Product History</h1>
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'all' | 'favorites')}>
          <TabsList data-design-id="history-tabs" className="w-full">
            <TabsTrigger data-design-id="tab-all" value="all" className="flex-1">
              <History className="w-4 h-4 mr-2" />
              All Scans
            </TabsTrigger>
            <TabsTrigger data-design-id="tab-favorites" value="favorites" className="flex-1">
            ...
            {filteredProducts.length > 0 ? (
              ...
              >
                ...
                <h3 data-design-id="empty-title" className="font-semibold mb-2">
                  {searchQuery ? 'No matching products' : activeTab === 'favorites' ? 'No favorites yet' : 'No products scanned'}
                </h3>
                <p data-design-id="empty-desc" className="text-sm text-muted-foreground">
                  {searchQuery 
