...
interface ProcessedMaskingTerm extends MaskingTerm {
  lowerTerm: string;
}

let classificationRules: CompiledRule[] = [];
let maskingTerms: ProcessedMaskingTerm[] = [];
...
export function classifyIngredient(
  ...
  if (
    normalizedResult.subIngredients &&
    normalizedResult.subIngredients.length > 0
  ) {
    productIngredient.subIngredients = normalizedResult.subIngredients.map(
      (sub) => classifyIngredient(sub, sourceProvenance),
    );

    const hasUnsafeSub = productIngredient.subIngredients.some(
      (s) => s.safetyStatus === "unsafe",
    );
    if (hasUnsafeSub && safetyStatus !== "unsafe") {
      productIngredient.safetyStatus = "unsafe";
      const unsafeSubs = productIngredient.subIngredients.filter(
        (s) => s.safetyStatus === "unsafe",
      );
...
function checkForMasking(text: string): {
  ...
  for (const term of maskingTerms) {
    if (lowerText.includes(term.lowerTerm)) {
      return {
        isMasking: true,
        reason: term.reason,
...
export function classifyIngredientsList(
  ...
  return normalizedResults.map((result) =>
    classifyIngredient(result, sourceProvenance),
  );
}
...
export function calculateProductSafety(
  ...
  const unsafeCount = allIngredients.filter(
    (i) => i.safetyStatus === "unsafe",
  ).length;
  const unknownCount = allIngredients.filter(
    (i) => i.safetyStatus === "unknown",
  ).length;
  const totalIngredients = allIngredients.length;

  const unsafeIngredients = allIngredients.filter(
    (i) => i.safetyStatus === "unsafe",
  );
  ...
  if (unsafeCount > 0) {
    ...
    maskingIngredients.some((m) => m.maskingRiskLevel === "high")
  ) {
  ...
  return {
    ...
    unknownPercentage:
      totalIngredients > 0
        ? Math.round((unknownCount / totalIngredients) * 100)
        : 0,
    unsafeIngredients,
    maskingIngredients,
    totalIngredients,
