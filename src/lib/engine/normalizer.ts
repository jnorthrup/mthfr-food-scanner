...
export function parseIngredientsList(rawText: string): string[] {
  ...
  text = text.replace(
    /contains\s*(?:less\s*than\s*)?[\d.]+%\s*(?:of\s*)?:?/gi,
    ",",
  );
...
export async function normalizeIngredientsList(
  rawText: string,
): Promise<NormalizationResult[]> {
  const parsed = parseIngredientsList(rawText);

  const normalizationPromises = parsed.map(async (ingredient) => {
  ...
  for (const item of normalizedResults) {
    ...
    const { result, isSubIngredient } = item;
