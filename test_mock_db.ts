import { normalizeIngredient, parseIngredientsList, initializeNormalizer } from "./src/lib/engine/normalizer";
import type { NormalizationResult } from "./src/types";

// Because `fuse.js` is imported, and our mock data is simple, `fuseIndex` doesn't exist, which causes the default `normalizeIngredient` to just return basic 0.5 confidence. But if we can create a delay simulating a network or heavy DB lookup, we can show Promise.all() working.

export async function normalizeIngredientMock(
  rawText: string,
): Promise<NormalizationResult> {
  const normalized = rawText.toLowerCase().trim();

  // Simulated async work that Promise.all will parallelize
...
export async function normalizeIngredientsListOptimized(
  ...
  const promises = parsed.map(ingredient => {
    ...
    return normalizeIngredientMock(cleanedIngredient).then(result => ({ isSubIngredient, result }));
  });
...
export async function normalizeIngredientsListOriginal(
  ...
  for (const ingredient of parsed) {
    ...
    if (cleanedIngredient) {
      const result = await normalizeIngredientMock(cleanedIngredient);
